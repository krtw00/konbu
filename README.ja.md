...
<p align="center">
  <strong>個人 AI ライフログ。</strong><br>
  CLI · MCP · セルフホスト · AIネイティブ
</p>

<p align="center">
  雑務・記録・思考を AI と一緒に Go バイナリ 1 個に積み上げ、 1 つの場所から検索する。<br>
...
konbu は **個人 AI ライフログ** -- セルフホスト可能な Go バイナリ 1 個で、 1 日に散らばる「雑務・記録・思考」 を AI と一緒に積み上げ、 1 つの検索インターフェイスで「ここ見れば全部書いてある」 を実現する個人ツールです。 Notion + Todoist + Calendar の代替ではなく、 **4 つのアプリを行き来して 1 つのことを探す行為そのもの**の代替です。

何が違うか:

- **MCP サーバー + CLI クライアント** -- AI エージェント (Claude / Cursor / 任意の MCP クライアント) と shell / スクリプト、 2 経路から konbu を操作できる。
- **横断全文検索** -- メモ・ToDo・予定・ブックマーク・構造化テーブルを 1 クエリで横断検索。 これは副次機能ではなく**コア UX**。
...
- **横断全文検索** -- メモ・ToDo・予定・ブックマーク・構造化テーブルを 1 クエリで横断検索 (コア UX)

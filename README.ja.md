...
<p align="center">
  <strong>開発者ツールのように作られたオンライン手帳。</strong><br>
  CLI · MCP · セルフホスト · AIネイティブ
</p>

<p align="center">
  Goバイナリ1個でNotion + Todoist + Calendarを置き換える。<br>
  Claude、Cursor、その他のMCPクライアントから直接操作可能。
</p>
...
konbuは、Notion + Todoist + Google Calendar をひとつにまとめた、セルフホスト可能なオンライン手帳です。

何が違うか:

- **MCPサーバー内蔵** -- Claude Desktop、Cursor、その他のMCPクライアントから、予定・メモ・ToDoを直接操作できる。「来週の空きに歯医者の予定を入れて」を自然言語で。
- **CLIクライアント同梱** -- ターミナルから `konbu memo add` や `konbu todo list` で操作できる。
- **BYOK対応** -- 自分のOpenAI/Anthropic APIキーを使う、または同梱の無料枠を使う。どちらでもOK。
- **横断全文検索** -- メモ・ToDo・予定・ブックマークをまたいで「あれどこだっけ」がなくなる。
- **セルフホスト可能** -- Goバイナリ1個、Docker compose、またはホスト版どれでも。

「予定・メモ・タスク・URLがあちこちのアプリに散らばってる状態」を、ひとつのGoバイナリで終わらせます。

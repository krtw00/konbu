...
> **更新メモ (2026-05-21)**: Tools（ブックマーク）は本番撤去済み（[concept.md](../01-overview/concept.md) / KONBU-19）。本プランの **ツール / ブックマーク検索の次元（`type: "tool"`、tools 検索 SQL、`idx_tools_*_trgm` マイグレーション、フィルタの「ブックマーク」）は無効** であり、対象は memo / todo / event / タグ + 曖昧検索 + ページネーションに限定する。詳細な再設計は KONBU-14 着手時に行う。

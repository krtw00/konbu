...
var eventCols = `id, user_id, title, description, start_at, end_at, all_day, recurrence_rule, recurrence_end, created_at, updated_at`

func scanEvent(row interface{ Scan(...any) error }) (EventRow, error) {
	var e EventRow
	err := row.Scan(&e.ID, &e.UserID, &e.Title, &e.Description, &e.StartAt, &e.EndAt, &e.AllDay, &e.RecurrenceRule, &e.RecurrenceEnd, &e.CreatedAt, &e.UpdatedAt)
	return e, err
}

func (q *Queries) CreateEvent(ctx context.Context, userID uuid.UUID, title, description string, startAt time.Time, endAt *time.Time, allDay bool, recurrenceRule, recurrenceEnd *string) (EventRow, error) {
	row := q.db.QueryRowContext(ctx,
		`INSERT INTO calendar_events (user_id, title, description, start_at, end_at, all_day, recurrence_rule, recurrence_end)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		 RETURNING `+eventCols,
		userID, title, description, startAt, endAt, allDay, recurrenceRule, recurrenceEnd)
	return scanEvent(row)
}

func (q *Queries) GetEventByID(ctx context.Context, id, userID uuid.UUID) (EventRow, error) {
	row := q.db.QueryRowContext(ctx,
		`SELECT `+eventCols+` FROM calendar_events WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL`, id, userID)
	return scanEvent(row)
}

...
export function CommandPalette({ onOpenMemo }: CommandPaletteProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setItems([
        ...(mm.data || []).map((m) => ({ type: 'memo' as const, title: m.title || 'Untitled', id: m.id })),
        ...(td.data || []).map((t) => ({ type: 'todo' as const, title: t.title, id: t.id })),
          ...
          {items.map((item) => {
            ...
            return (
              <CommandItem key={`${item.type}-${item.id}`} onSelect={() => handleSelect(item)}>
                ...
                <span className="text-xs text-muted-foreground mr-2 w-10">{item.type}</span>
                {item.title}
              </CommandItem>
            )
          })}
